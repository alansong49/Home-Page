...
def _load_config():
    ...
    # 首先尝试从环境变量加载配置（环境变量优先级最高）
    env_config = {}
    if os.environ.get('GITHUB_URL'):
        env_config['github_url'] = os.environ.get('GITHUB_URL')
    if os.environ.get('GITHUB_TOKEN'):
        env_config['github_token'] = os.environ.get('GITHUB_TOKEN')
    if os.environ.get('NAME'):
        env_config['name'] = os.environ.get('NAME')
    if os.environ.get('BIO'):
        env_config['bio'] = os.environ.get('BIO')
    if os.environ.get('DARK_MODE'):
        env_config['dark_mode'] = os.environ.get('DARK_MODE')
    if os.environ.get('INTRODUCTION_FILE'):
        env_config['introduction_file'] = os.environ.get('INTRODUCTION_FILE')
    
    # 全局变量config已在函数开头声明，以下所有赋值都会影响全局变量
    try:
        # 尝试从config.json读取配置
...
def make_github_request(url, timeout=5):
    try:
        ...
        if os.environ.get('GITHUB_TOKEN'):
            ...
            print("从环境变量GITHUB_TOKEN获取令牌")
        else:
            # 然后尝试从github_token.txt文件中读取令牌
...
def get_readme_content(username):
    try:
        ...
        readme_response = requests.get(readme_url_main, timeout=5, proxies={"http": None, "https": None})
        ...
        readme_response = requests.get(readme_url_master, timeout=5, proxies={"http": None, "https": None})
...
def index():
    ...
    possible_paths = [
        ...
        os.path.join(os.getcwd(), 'default', background_image)  # default目录
    ]
